...
function provedorCtrl($scope, $rootScope, $location, $interval, Provedores,  webStorage) {
    ...
    $scope.inicio = function(){ 
    	...
    	if (webStorage.get('provedor') == null) {
    ...
    $rootScope.guardar = function(){
    	...
    	if ($scope.provForm.$valid) {
            ...
            $rootScope.guardando = true;
                ...
                Provedores.save($scope.datos,function (data){
...
provedorCtrl.$inject = ['$scope', '$rootScope', '$location', '$interval' , 'Provedores',  'webStorage'];

//asignamos a la aplicacion la funcion
app.controller('provedorCtrl', provedorCtrl);
